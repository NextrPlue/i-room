...
> 관리자, 근로자, 시스템 계정 등 다양한 사용자 유형을 지원하도록 설계하였으며, 관리자 3종(SUPER_ADMIN, ADMIN, READER), 시스템 계정 4종(ENTERANCE, EQUIPMENT,
> WORKER, PPE)으로 역할을 세부화하였습니다.
>
> 이를 통해 마이크로서비스 별로 세밀한 API 접근 제한이 가능하도록 하였습니다.

### 📌 데이터 보안 및 무결성

> 비밀번호 정책, 커스텀 데이터 유효성 검증(@ValidPassword, @ValidPhone), 비밀번호 암호화, 그리고 개인정보 마스킹 기능을 통해 사용자 데이터의 무결성과 보안을 고려했습니다.
>
> 이를 통해 민감한 사용자 정보를 안전하게 관리합니다.
