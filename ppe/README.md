...
- **AI 기반 PPE 탐지**: YOLOv8 모델을 활용한 안전모/안전고리 착용 여부 실시간 탐지
- **다중 객체 추적**: BoT-SORT 알고리즘을 통한 근로자별 연속적 추적 및 ID 관리
...
3. **환경 변수 설정**
   ...
   RTSP_URL=ppe/helmet_off.mp4        # 비디오 소스 (RTSP URL 또는 파일)
...
- **YOLOv8 모델**: 안전모/안전고리 착용/미착용 4개 클래스 탐지
...
- **ON-근접 억제**: 착용 감지 직후 미착용 오탐 방지

### 4. 위반 관리 및 알림

- **실시간 알림**: 위반 감지 시 즉시 알림 시스템 연동
- **중복 방지**: 동일 위반에 대한 반복 알림 억제
- **상태 추적**: 위반 상태의 시작/지속/해제 관리
...
- **탐지 클래스**:
    ...
    - `helmet_off`: 안전모 미착용
    - `harness_on`: 안전고리 착용
    - `harness_off`: 안전고리 미착용
- **신뢰도 임계값**: 동적 조정 (기본 0.4)
