...
input_root = os.path.join(base_dir, FOLDER_NAME)
output_root = os.path.join(base_dir, "1_completed")
split_set = ["train", "val"]
...
for split in split_set:
    ...
    # 현재 split(train / val)에 해당하는 목표 수량
    ...
    for json_file in os.listdir(in_labels_dir):
        ...
        # 객체 개수 조건 먼저 적용
        if len(annotations) < MIN_OBJECTS:
            continue
